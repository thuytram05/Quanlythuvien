...
def create_borrow_receipt(user_id, cart_items):
    if cart_items:
        phieu = PhieuMuon(ma_nguoi_dung=user_id, han_tra=datetime.now() + timedelta(days=14))
        ...
        for item in cart_items:
            ...
            if not sach or sach.so_luong_con < 1:
                raise Exception(f'Sách "{sach.ten_sach}" đã hết bản!')
